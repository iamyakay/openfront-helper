...
export interface SimpleGraphProps {
  ...
  className?: string;
  /**
   * vertex: dots on each sample (default).
   * scrub: no vertex dots; hover the plot to show one dot + tooltip (interpolated value).
   */
  interactionMode?: "vertex" | "scrub";
}
...
const SimpleGraph = ({
  ...
  const { points, pathD, scrubMetrics } = useMemo(() => {
    if (!data || data.length === 0) {
      return { points: [], pathD: "", scrubMetrics: null };
    }
    ...
    const paddedRange = Math.max(paddedMax - paddedMin, 1e-9);
  ...
  const showVertexDots = showDots && interactionMode === "vertex";

  const handleScrubMove = useCallback(
    (event: React.MouseEvent<SVGRectElement>) => {
      if (!svgRef.current || !scrubMetrics || !data?.length) return;
      ...
      const { graphPadding, graphWidth, graphHeight, paddedMin, paddedRange, n } =
        scrubMetrics;
      const xClamped = Math.max(
        graphPadding,
        Math.min(graphPadding + graphWidth, p.x),
      );
      if (n <= 1) {
        ...
        const y =
          graphPadding +
          graphHeight -
          ((value - paddedMin) / paddedRange) * graphHeight;
        setScrub({
          x: graphPadding + graphWidth / 2,
          y,
          value,
          label: data[0].label,
        });
      ...
      const t = ((xClamped - graphPadding) / graphWidth) * (n - 1);
      const i0 = Math.floor(t);
      const i1 = Math.min(n - 1, i0 + 1);
      const f = t - i0;
      const value = data[i0].value * (1 - f) + data[i1].value * f;
      const y =
        graphPadding +
        graphHeight -
        ((value - paddedMin) / paddedRange) * graphHeight;
      const x = graphPadding + (t / (n - 1)) * graphWidth;
      const label =
        f < 0.5
          ? (data[i0].label ?? data[i1].label)
          : (data[i1].label ?? data[i0].label);
      setScrub({ x, y, value, label });
    },
    [data, scrubMetrics],
  );
  ...
  return (
    <div
      ref={containerRef}
      className={cn(
        "relative flex min-h-0 w-full items-center justify-center text-gray-900 dark:text-gray-100",
        className,
      )}
    ...
    >
      <div
        ...
        style={{
          width: `min(100%, ${Number(height) * 2}px)`,
          aspectRatio: "800 / 400",
          maxHeight: `${height}px`,
        }}
      >
        <svg
          ...
          className="block h-full w-full text-gray-900 dark:text-gray-100"
        ...
        {interactionMode === "scrub" && scrubMetrics && data.length > 0 ? (
          <rect
            x="40"
            y="40"
            width="720"
